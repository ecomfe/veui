...
describe('utils/math', () => {
  it('should calculate equality correctly and distinguish -0 vs +0', () => {
    ...
    expect(is(-1, 0)).to.be.equal(false)
    expect(is(1, -0)).to.be.equal(false)
    expect(is(1, 1)).to.be.equal(true)
    expect(is(0, 0)).to.be.equal(true)
    expect(is(-0, -0)).to.be.equal(true)
    expect(is(-0, 0)).to.be.equal(false)
    expect(is(0, -0)).to.be.equal(false)
    expect(is(Infinity, Infinity)).to.be.equal(true)
  ...
  it('should identify positive values as expected', () => {
    expect(isPositive(Infinity)).to.be.equal(true)
    expect(isPositive(1)).to.be.equal(true)
    expect(isPositive(0)).to.be.equal(true)
